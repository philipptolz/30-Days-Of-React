...
const filteredPriceArray = priceArray.filter(
  (prize) => typeof prize === "number"
);

const priceArraySum = filteredPriceArray.reduce((prize, sum) => (sum += prize));

const firstWithoutPrice = products.find(
  (product) => typeof product.price != "number"
);

const firstWithoutPriceIndex = products.findIndex(
  (product) => typeof product.price != "number"
);
